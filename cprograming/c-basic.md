...
在程序的指导下，程序可以做很多事情，比如数值计算，发邮件，对话，甚至做录音和视频。要完成这些工作，程序使用的最基本的载体就是 `数据`，数据分为两种，即 **常量和变量**

有些数据在程序使用之前就已经被设定好了，在整个过程中没有变化（这段话描述不准确，但是为了通俗易懂，暂且这么描述），这些就被称为 `常量(constant)`。其他数据类型在程序执行期间进行重新赋值或者改变，那么这些就被称为 `变量(variable)`。例如 `int number` 就是一个变量，`3.1415` 就是一个常量。

### 数据类型

我们上面说到，数据类型的关键字一共分为 12 种，下面我们就来更全面的了解一下这些数据类型，在了解数据类型之前，我们需要先了解一下这些概念 **位、字节和字**

位、字节和字都是对计算机存储单元的描述。在计算机世界中，最小的单元是 `位(bit)`，一个位可以表示一个 0 或 1

>在计算机硬件中，通常使用逻辑门来表示 0 和 1 ，类似的还有与或非门。
