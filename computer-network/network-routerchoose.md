...
OSPF 是为了克服 RIP 的缺点在 1989 年开发出来的，OSPF 称为 **开放最短路径优先 ( Open Shortest Path First )** 协议。注意虽然它被叫做最短路径优先协议，但是却并不能说明其他协议不是最短路径优先的，一般自治系统内的路由器都会选择一个最短路径。

OSPF 使用的是**分布式的链路状态协议**，而非像 RIP 那样的距离向量协议。
