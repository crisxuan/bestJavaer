...
当我们计算机接入网络时，我们通常都会和 ISP（因特网服务提供商） 打交道，我们的计算机需要经过 ISP 才能联网，这个联网的过程就是 ISP 和我们主机建立点对点连接的过程。
...
*PPPoE(Pointer-to-Pointer over Ethernet)* 可以称作以太网上的 PPP 协议，也是在链路层使用的一种协议。由于 PPP 协议的一个重要作用是提供了身份验证功能，但是 PPP 帧中并没有提供地址，而以太网是一种广播类型的局域网，虽然简单易用并且使用成本低，但是以太网并不具备验证身份的功能，因此通信时不安全的。

而 PPPoE 解决了上面这两个协议的痛点，PPPoE 通过将 PPP 帧进行封装，从而实现以太网上的点对点通信，并且提供了身份验证、加密和压缩功能，而且 PPPoE 帧中海油一个 *Session_ID* 很好的保证了用户的安全性。下面是 PPPoE 的报文：
