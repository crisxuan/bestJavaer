...
![](https://imgkr.cn-bj.ufileos.com/077ad111-fd61-477d-a6a7-4518bf9aaafd.png)
...
![本文提纲](https://imgkr.cn-bj.ufileos.com/f8a75556-6947-4fff-9641-7437dd4192b5.png)
...
![半连接队列与全连接队列](https://imgkr.cn-bj.ufileos.com/ff7b0667-a0b3-4686-958d-1662eb77991d.png)
...
![](https://imgkr.cn-bj.ufileos.com/639b5630-83c3-4823-a802-9510ef593385.png)


在「LISTEN 状态」时，`Recv-Q/Send-Q` 表示的含义如下：

![](https://imgkr.cn-bj.ufileos.com/a62f572a-f4ed-4bef-bac2-27430eac6a67.png)
...
![](https://imgkr.cn-bj.ufileos.com/d51db075-bdcc-4d5b-ba42-d71de3de440f.png)
...
![测试环境](https://imgkr.cn-bj.ufileos.com/fefe24ad-6cc6-4386-a472-35f369c08435.png)
...
![](https://imgkr.cn-bj.ufileos.com/fd4897e4-5d31-407e-9447-65d5cf8abed3.png)


在服务端可以使用 `ss` 命令，来查看当前 TCP 全连接队列的情况：

![](https://imgkr.cn-bj.ufileos.com/26833da1-6a0e-41b9-a1d9-de960f6a5f43.png)

其间共执行了两次 ss 命令，从上面的输出结果，可以发现当前 TCP 全连接队列上升到了 129 大小，超过了最大 TCP 全连接队列。

**当超过了 TCP 最大全连接队列，服务端则会丢掉后续进来的 TCP 连接**，丢掉的 TCP 连接的个数会被统计起来，我们可以使用 netstat -s 命令来查看：

![](https://imgkr.cn-bj.ufileos.com/01803f19-b061-40ea-8eed-05466a7e11af.png)
...
![全连接队列溢出](https://imgkr.cn-bj.ufileos.com/fb25464a-0306-44a6-9cef-0075e64b9f0e.png)

> Linux 有个参数可以指定当 TCP 全连接队列满了会使用什么策略来回应客户端。

实际上，丢弃连接只是 Linux 的默认行为，我们还可以选择向客户端发送 RST 复位报文，告诉客户端连接已经建立失败。

![](https://imgkr.cn-bj.ufileos.com/a08d38aa-fb91-466c-bb08-52d5f2820bfc.png)
...
![](https://imgkr.cn-bj.ufileos.com/18a6c99b-40ce-49c9-8686-9731747563d6.png)
...
![](https://imgkr.cn-bj.ufileos.com/4415889d-50e6-4db5-a4e5-2f8ff818aca9.png)


现在我们重新压测，把 TCP 全连接队列**搞大**，把 `somaxconn` 设置成 5000：

![](https://imgkr.cn-bj.ufileos.com/6f35fac4-dc59-4123-bad3-79491f1fd819.png)

接着把 Nginx 的 backlog 也同样设置成 5000：

![](https://imgkr.cn-bj.ufileos.com/880facf1-69ae-46a0-953b-5a6e58e0883b.png)

最后要重启 Nginx 服务，因为只有重新调用 `listen()` 函数 TCP 全连接队列才会重新初始化。

重启完后 Nginx 服务后，服务端执行 ss 命令，查看 TCP 全连接队列大小：

![](https://imgkr.cn-bj.ufileos.com/99bcb281-ea8e-414a-b440-e6826e3a779f.png)
...
![](https://imgkr.cn-bj.ufileos.com/8239acb1-95f4-45c5-90d7-a5bb8cb44255.png)

从上面的执行结果，可以发现全连接队列使用增长的很快，但是一直都没有超过最大值，所以就不会溢出，那么 `netstat -s` 就不会有 TCP 全连接队列溢出个数的显示：

![](https://imgkr.cn-bj.ufileos.com/88ae59fb-27b6-40c7-b567-2d4f1ef5451d.png)
...
![](https://imgkr.cn-bj.ufileos.com/eac839c7-6325-4fcb-885e-673da156e6c3.png)
...
![测试环境](https://imgkr.cn-bj.ufileos.com/6f760794-2964-4f60-af2d-afa686edadd5.png)
...
![](https://imgkr.cn-bj.ufileos.com/7861ed17-5a0d-48b7-b27e-89074d813440.png)

当服务端受到 SYN 攻击后，连接服务端 ssh 就会断开了，无法再连上。只能在服务端主机上执行查看当前 TCP 半连接队列大小：

![](https://imgkr.cn-bj.ufileos.com/90e9aa9c-e1dc-4012-aa45-049085f01d81.png)

同时，还可以通过 netstat -s 观察半连接队列溢出的情况：

![](https://imgkr.cn-bj.ufileos.com/dc03c52b-29ed-4a77-965e-9edf8d2a3f22.png)
...
![](https://imgkr.cn-bj.ufileos.com/c1f4d40a-6889-461b-b92b-fc3e5cee06d0.png)
...
![](https://imgkr.cn-bj.ufileos.com/b272f1b9-fac2-4288-b07b-b799afcfb8d4.png)

从源码中，我可以得出共有三个条件因队列长度的关系而被丢弃的：

![](https://imgkr.cn-bj.ufileos.com/18481a31-9252-4fcc-b6b2-326d2daed0df.png)
...
![](https://imgkr.cn-bj.ufileos.com/536f7465-c1af-48ce-82a1-f489687cc15c.png)
...
![](https://imgkr.cn-bj.ufileos.com/e8385a98-5acb-40f5-968b-eb5124ec60d5.png)

从上面的代码中，我们可以算出 max_qlen_log 是 8，于是代入到 检测半连接队列是否满的函数 reqsk_queue_is_full ：

![](https://imgkr.cn-bj.ufileos.com/5b759d77-6e49-4778-a706-0d9d4b000624.png)
...
![](https://imgkr.cn-bj.ufileos.com/1c04a5d9-cfbf-4a02-8730-af8bbcaf929c.png)
...
![](https://imgkr.cn-bj.ufileos.com/4685c08f-ac0e-4f8d-b87a-97102ec96b75.png)

配置完后，服务端要重启 Nginx，因为全连接队列最大和半连接队列最大值是在 listen() 函数初始化。

根据前面的源码分析，我们可以计算出半连接队列 max_qlen_log 的最大值为 256：

![](https://imgkr.cn-bj.ufileos.com/5b8b6295-46f6-48c4-b58d-6494f742475f.png)

客户端执行 hping3 发起 SYN 攻击：

![](https://imgkr.cn-bj.ufileos.com/0f582a66-3030-4a04-b529-8377f2156bb5.png)

服务端执行如下命令，查看处于 SYN_RECV 状态的最大个数：

![](https://imgkr.cn-bj.ufileos.com/70810025-a971-42c3-ad96-baa2a5d368f3.png)
...
![](https://imgkr.cn-bj.ufileos.com/d85c0015-4a38-439d-8926-331c768eebd9.png)
...
![](https://imgkr.cn-bj.ufileos.com/58e3c3ef-ceb6-42da-882d-ec4ce7081d6d.png)
...
![开启 syncookies 功能](https://imgkr.cn-bj.ufileos.com/28a6ced8-7e3d-4de8-87b3-11139f9af380.png)
...
![](https://imgkr.cn-bj.ufileos.com/bf3d0431-29ef-46e0-be15-216593fd04c1.png)
...
![](https://imgkr.cn-bj.ufileos.com/badba1bf-692f-43dc-9ac7-3042f0bbee61.png)

增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：

![](https://imgkr.cn-bj.ufileos.com/02d8b91e-fef9-4c31-bb0e-d69ed60df18f.png)
...
![](https://imgkr.cn-bj.ufileos.com/cccd22b4-e4ad-44bf-a366-a0847d55e29c.png)
...
![](https://imgkr.cn-bj.ufileos.com/4d25725d-c42a-40ee-a4b1-723dbc520a53.png)
